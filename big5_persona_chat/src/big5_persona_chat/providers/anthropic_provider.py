"""Anthropic (Claude Sonnet 4.5 等) プロバイダ。logprobs / seed 非対応。"""
...
def _split_system(messages: list[dict[str, str]]) -> tuple[str, list[dict[str, str]]]:
    """Claude API では system はトップレベル引数として渡す必要がある。"""
...
class AnthropicProvider(LLMProvider):
    """Anthropic Messages API の非同期ラッパ。"""
    ...
    async def complete(
        ...
        *,
        ...
        top_p: float = 0.95,
        seed: int | None = None,  # Claude には seed パラメータがないため無視
        max_tokens: int = 400,
        logprobs: bool = False,  # 無視
        top_logprobs: int = 0,
