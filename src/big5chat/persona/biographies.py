"""Biographic description pool for "persona thickness".

ConstructionPlan.md §C.4 recommends 50 biographies sampled randomly to prevent
literal adjective responses. This module provides a starter set of 20 per
language (EN, JA, ZH).

To extend to the full 50:
    - EN: use PersonaChat (Zhang et al. 2018) public dataset.
    - JA/ZH: craft additional culturally-natural descriptions.
"""
...
# Simplified Chinese biographies (Mainland-oriented, culturally natural).
...
def get_biography(language: str, idx: int) -> str:
    """Retrieve a biography by index, wrapping around if out of range."""
    if language == "zh":
        pool = BIOGRAPHIES_ZH
    elif language == "ja":
        pool = BIOGRAPHIES_JA
